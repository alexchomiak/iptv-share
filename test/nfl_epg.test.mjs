...
function fixture({ localTitle = "NFL | 08", eventDate = "2026-09-13T17:00:00Z", available = true } = {}) {
  ...
  const xml = `<tv><channel id="slot08"><display-name>${label}</display-name></channel>${["040000", "060000", "080000"].map(time => `<programme channel="slot08" start="20260913${time} +0000"><title>${label}</title></programme>`).join("")}</tv>`;
  const context = vm.createContext({
    Date: FixedDate, Intl, Buffer, XMLParser, gunzipSync,
...
test("unverified source assignment does not fabricate a game", async () => {
